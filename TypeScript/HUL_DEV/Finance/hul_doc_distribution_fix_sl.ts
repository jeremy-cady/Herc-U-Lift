...
function addColumns(sub: serverWidget.Sublist) {
...
function parseEmails(v: unknown): string[] {
    ...
    for (const m of matches) uniq.add(m.trim().toLowerCase());
    return Array.from(uniq);
}

function domainOf(normalizedEmail: string): string {
    const at = normalizedEmail.lastIndexOf('@');
    return at >= 0 ? normalizedEmail.substring(at + 1) : '';
}

function domainsOfEmails(emails: string[]): Set<string> {
...
function pickEmailForCustomer(ddEmails: string[], customerEmail: string, contactEmail: string): string {
    if (ddEmails.length === 0) return '';
    if (customerEmail && ddEmails.includes(customerEmail)) return '';
    const custDom = domainOf(customerEmail);
    if (custDom) {
        const m = ddEmails.find((e) => domainOf(e) === custDom);
        if (m) return m;
    }
    if (contactEmail && ddEmails.includes(contactEmail)) return contactEmail;
    return ddEmails[0];
}

type Row = Record<string, string | number | null>;

function rowPassesFilter(r: Row, hidden: Set<string>): boolean {
    ...
    if (docId && hidden.has(docId)) return false;

    const ddDismissedVal = (r['dd_dismissed'] ?? '').toString().toUpperCase();
    if (ddDismissedVal === 'T' || ddDismissedVal === 'TRUE' || ddDismissedVal === '1') return false;

    // Doc vs Customer domain (exclude if ANY DD email shares customer domain)
    if (customerDomain && ddDomains.has(customerDomain)) return false;

    // Contact vs Customer exact match
    if (contactEmail && customerEmail && contactEmail === customerEmail) return false;
    ...
    if (ddCustomerId && ddCustomerEmail && ddEmails.length > 0) {
        if (ddEmails.includes(ddCustomerEmail)) return false;
    }

    // All blank
    if (ddEmails.length === 0 && !contactEmail && !customerEmail) return false;
...
export function onRequest(ctx: EntryPoints.Suitelet.onRequestContext) {
    try {
        ...
        const fpageFromField = req.parameters.custpage_fpage as string | undefined;
        const fpageFromUrl = req.parameters.fpage as string | undefined;
        const fpageParam = fpageFromField || fpageFromUrl;
        const requestedIndex = Math.max(0, isFinite(+fpageParam!) ? parseInt(fpageParam as string, 10) - 1 : 0);

        // Hidden ids persisted in a hidden textarea (session-level)
        let hiddenIds = new Set<string>();
        const hiddenCsv = req.parameters.custpage_hidden as string | undefined;
        if (hiddenCsv) {
            for (const part of hiddenCsv.split(',')) {
                const v = part.trim();
                if (v) hiddenIds.add(v);
            }
        }
        ...
        if (req.method === 'POST') {
            const lineCount = (req as any).getLineCount
                ? (req as any).getLineCount({ group: 'custpage_results' }) as number
                : 0;

            const clearHidden = (req.parameters.custpage_clear_hidden as string | undefined) === 'T';
            if (clearHidden) hiddenIds = new Set();

            for (let i = 0; i < lineCount; i += 1) {
                const hide = (req as any).getSublistValue
                    ? (req as any).getSublistValue({ group: 'custpage_results', name: 'hide_line', line: i }) as string
                    : 'F';
                if (hide === 'T') {
                    const ddId = (req as any).getSublistValue({ group: 'custpage_results', name: 'doc_dis_id', line: i }) as string;
                    if (ddId) hiddenIds.add(ddId);
                }

                const dis = (req as any).getSublistValue
                    ? (req as any).getSublistValue({ group: 'custpage_results', name: 'dismiss', line: i }) as string
                    : 'F';
                if (dis === 'T') {
                    const ddId = (req as any).getSublistValue({ group: 'custpage_results', name: 'doc_dis_id', line: i }) as string;
                    if (ddId) {
                        try {
                            ...
                            dismissFailed += 1;
                            updateSummary.push(`FAILED dismiss DD ${ddId}: ${(err as Error).message}`);
                        }
                    }
                }

                const apply = (req as any).getSublistValue
                    ? (req as any).getSublistValue({ group: 'custpage_results', name: 'apply_email', line: i }) as string
                    : 'F';
                if (apply === 'T') {
                    const custId = (req as any).getSublistValue({ group: 'custpage_results', name: 'target_customer_id', line: i }) as string;
                    const targetEmail = (req as any).getSublistValue({ group: 'custpage_results', name: 'target_email', line: i }) as string;
                    const currentEmail = normEmail((req as any).getSublistValue({ group: 'custpage_results', name: 'customer_email', line: i }) as string);
                    const docId = (req as any).getSublistValue({ group: 'custpage_results', name: 'doc_dis_id', line: i }) as string;
                    ...
                    try {
                        ...
                        failed += 1;
                        updateSummary.push(`FAILED: cust ${custId} ← "${targetEmail}" (DD ${docId}) :: ${(err as Error).message}`);
                    }
                }
            }
        }
        ...
        const clearHiddenChecked = (req.parameters.custpage_clear_hidden as string | undefined) === 'T';
        form.addField({
        ...
        for (let rawPageIdx = 0; rawPageIdx < rawTotalPages; rawPageIdx += 1) {
            ...
            const rows = page.data.asMappedResults() as Row[];

            for (const r of rows) {
                if (!rowPassesFilter(r, hiddenIds)) continue;

                // Count every passing row
                filteredTotalRows += 1;

                // Only render rows that fall into the current filtered page window
                if (filteredTotalRows <= filteredStartRow) continue;
                if (displayed >= PAGE_SIZE) continue;
                ...
                if (rawDdEmails) sub.setSublistValue({ id: 'doc_dis_email', line: displayed, value: rawDdEmails });

                const contactId = get('contact_id');
                if (contactId) sub.setSublistValue({ id: 'contact_id', line: displayed, value: contactId });
                const contactName = get('contact_name');
                if (contactName) sub.setSublistValue({ id: 'contact_name', line: displayed, value: contactName });
                const contactEmail = get('contact_email');
                if (contactEmail) sub.setSublistValue({ id: 'contact_email', line: displayed, value: contactEmail });

                const customerId = get('customer_id');
                if (customerId) sub.setSublistValue({ id: 'customer_id', line: displayed, value: customerId });
                const customerName = get('customer_name');
                if (customerName) sub.setSublistValue({ id: 'customer_name', line: displayed, value: customerName });
                const customerEmail = get('customer_email');
                if (customerEmail) sub.setSublistValue({ id: 'customer_email', line: displayed, value: customerEmail });
                ...
                if (targetCustomerId) sub.setSublistValue({ id: 'target_customer_id', line: displayed, value: targetCustomerId });
                if (chosen) sub.setSublistValue({ id: 'target_email', line: displayed, value: chosen });
