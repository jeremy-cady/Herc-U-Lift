...
function audit(title: string, details: unknown): void {
    try {
        log.audit(title, details as any);
    } catch {
        /* no-op */
    }
}

function errorLog(title: string, details: unknown): void {
    try {
        log.error(title, details as any);
    } catch {
        /* no-op */
    }
}
...
function formatUtcIsoToMMDDYYYY(iso: string): string {
    try {
        if (!iso) return '';
...
function formatDateMMDDYYYY(v: unknown): string {
    try {
        if (!v) return '';
        let d: Date;
        const s = String(v);
...
function readFiltersFromParams(p: Record<string, string | number | boolean | null | undefined>): Filters {
    ...
    if (!f.tranid) delete f.tranid;
    if (!f.trandateFrom) delete f.trandateFrom;
    if (!f.trandateTo) delete f.trandateTo;
    if (!f.customer) delete f.customer;
...
function containsInsensitive(hay: unknown, needle: string | undefined): boolean {
...
function parseDateLoose(v: unknown): number {
    if (v == null) return NaN;
    const d = new Date(String(v));
    return isNaN(d.getTime()) ? NaN : d.getTime();
}

function applyFilters(rows: any[], filters: Filters): any[] {
    try {
        if (!rows || rows.length === 0) return rows || [];

        const hasDateRange =
            Boolean(filters.trandateFrom && filters.trandateFrom.trim()) ||
        ...
        const filtered = rows.filter((row) => {
            ...
            if (hasDateRange) {
                ...
                if (!isNaN(rowMs)) {
                    if (!isNaN(fromMs) && rowMs < fromMs) return false;
                    if (!isNaN(toMs) && rowMs > toMs) return false;
                }
            }
        ...
        errorLog('Filters:apply:error', (e as any)?.message ?? String(e));
        return rows || [];
    }
}
...
export function onRequest(ctx: EntryPoints.Suitelet.onRequestContext): void {
    try {
        ...
        errorLog('SL:error', (e as any)?.message ?? String(e));
        ctx.response.write(`Unexpected error: ${(e as any)?.message ?? String(e)}`);
    }
}

/* --------------------------- ACTION HANDLERS --------------------------- */

function handleRebuildRequest(
    ctx: EntryPoints.Suitelet.onRequestContext,
    oldFileId: number,
    runToken: string
): void {
    try {
        ...
        errorLog('MR:start:error', (e as any)?.message ?? String(e));
        ctx.response.write(`Error starting rebuild: ${(e as any)?.message ?? String(e)}`);
    }
}

function handlePoll(
    ctx: EntryPoints.Suitelet.onRequestContext,
    oldFileIdParam: number
): void {
    try {
        ...
        const pctRaw =
            (st as any)?.percentage ??
            (st as any)?.percentageCompleted ??
            (st as any)?.percentCompleted;
        ...
        try {
            ...
            errorLog('File:delete:error', {
                ...
                error: (delErr as any)?.message ?? String(delErr)
            });
        }

        // 2) Find newest dataset JSON by internalid DESC
        let newFileId: number | null = null;
        let newestModifiedIso: string | null = null;
        ...
        try {
            ...
            errorLog('File:newest:error', (findErr as any)?.message ?? String(findErr));
        }

        // 3) Update deployment params and redirect back with new file
        if (Number.isFinite(newFileId as number) && (newFileId as number) > 0) {
            ...
            if (dInfo.internalId) {
                try {
                    ...
                    errorLog('DeploymentParam:update:error', (e as any)?.message ?? String(e));
                }
                ...
                try {
                    ...
                    const readBack = String(
                        dRec.getValue({ fieldId: SCRIPT_PARAM_LAST_REBUILD }) || ''
                    ).trim();
                    ...
                    errorLog('LastRebuild:update:error', (e as any)?.message ?? String(e));
                }
            } else {
        ...
        const cur = runtime.getCurrentScript();
        const redirectOpts: http.SendRedirectOptions = {
        ...
        errorLog('Poll:error', (e as any)?.message ?? String(e));
        ctx.response.write(`Polling error: ${(e as any)?.message ?? String(e)}`);
    }
}
...
function renderMainPage(
    ...
    try {
        ...
        if (filters.tranid) csvParams.custpage_f_tranid = filters.tranid;
        if (filters.trandateFrom) csvParams.custpage_f_trandate_from = filters.trandateFrom;
        if (filters.trandateTo) csvParams.custpage_f_trandate_to = filters.trandateTo;
        if (filters.customer) csvParams.custpage_f_customer = filters.customer;
        if (filters.location) csvParams.custpage_f_location = filters.location;
        ...
        if (Number.isFinite(fileIdParam)) {
            try {
                ...
                errorLog('Render:load:error', (e as any)?.message ?? String(e));
                htmlFld.defaultValue = `
                    <div class="hul-lease-ui">
        ...
        errorLog('Render:error', (e as any)?.message ?? String(e));
        ctx.response.write(`Render error: ${(e as any)?.message ?? String(e)}`);
    }
}
...
function writeCsv(
    ...
    try {
        ...
        const f = file.load({ id: fileIdParam });
        const text = f.getContents() || '[]';
        const data = JSON.parse(text) as Array<Record<string, any>>;
        ...
        errorLog('CSV:error', (e as any)?.message ?? String(e));
        ctx.response.addHeader({ name: 'Content-Type', value: 'text/plain; charset=utf-8' });
        ctx.response.write(`CSV error: ${(e as any)?.message ?? String(e)}`);
    }
}

function renderHtmlTable(data: any[]): string {
    try {
        ...
        errorLog('Render:table:error', (e as any)?.message ?? String(e));
...
function injectThemeAndToolbarJs(form: serverWidget.Form): void {
    try {
        const cssAndJs = `
<style>
/* ---------- Global layout / title ---------- */
...
(function(){
    ...
    window.HUL_applyFilters = function(){
        ...
        errorLog('Theme:inject:error', (e as any)?.message ?? String(e));
    }
}

function renderToolbarHtml(): string {
    return `
...
function readParamFileId(): number {
    try {
        const raw = runtime.getCurrentScript().getParameter({ name: SCRIPT_PARAM_FILEID }) as
            | string
            | number
            | null;
        ...
        errorLog('Param:fileId:error', (e as any)?.message ?? String(e));
        return NaN;
    }
}

function readLastRebuildFromDeployment(): string {
    try {
        ...
        errorLog('LastRebuild:read:error', (e as any)?.message ?? String(e));
        return '';
    }
}
...
function getDeploymentInfo(): {
    ...
    try {
        ...
        errorLog('Deploy:resolve:error', (e as any)?.message ?? String(e));
...
function stageToPercent(stage: string): number {
    ...
    if (s.includes('GET_INPUT')) return 10;
    if (s.includes('MAP')) return 50;
    if (s.includes('SHUFFLE')) return 75;
    if (s.includes('REDUCE')) return 90;
    if (s.includes('SUMMARIZE')) return 98;
    return 15;
}