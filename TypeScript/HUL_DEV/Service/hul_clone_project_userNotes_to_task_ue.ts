...
function isJobId(id: number | null): boolean {
    if (!id) return false;
    try {
...
function cloneNoteToTask(params: {
    ...
    // Optionals
    if (isNonEmptyId(notetype)) n.setValue({ fieldId: 'notetype', value: Number(notetype) });
    if (isNonEmptyId(direction)) n.setValue({ fieldId: 'direction', value: Number(direction) });
    ...
    (n as any).setValue({ fieldId: 'activity', value: taskId });
...
const afterSubmit: EntryPoints.UserEvent.afterSubmit = (ctx) => {
    try {
        ...
        const isProjectContext =
            (attachedRecordId ? isJobId(attachedRecordId) : false) ||
