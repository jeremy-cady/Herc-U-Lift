...
/** ===== CONFIG ===== */
const FORM_ID = '121';
const TERMS_REQUIRE_CC = new Set<string>(['8']);
const DEBOUNCE_MS = 250;

/** ===== INTERNAL STATE ===== */
let debounceId: number | null = null;      // for warnings/checks
let debounceLogId: number | null = null;   // for unconditional PI logging
let lastWarnedKey = '';
...
/** ===== UTILS ===== */
function getStr(currentRecord: any, fieldId: string): string {
    try {
...
function logPaymentInstruments(custRec: record.Record): void {
    try {
        const n = custRec.getLineCount({ sublistId: 'paymentinstruments' }) || 0;
        clog('PI.count =', n);
        for (let i = 0; i < n; i += 1) {
            const tId = String(
                custRec.getSublistValue({
        ...
        clog('PI.log error:', (e as any)?.message || e);
    }
}

function hasCreditCard(custRec: record.Record): boolean {
    try {
        const n = custRec.getLineCount({ sublistId: 'paymentinstruments' }) || 0;
        for (let i = 0; i < n; i += 1) {
            const tId = String(
 ...
 */
function evaluate(currentRecord: any): { required: boolean; missing: boolean } {
    // Guard: correct form and a customer selected
    const formId = getStr(currentRecord, 'customform');
    if (formId !== FORM_ID) return { required: false, missing: false };

    const entityId = getStr(currentRecord, 'entity');
    if (!entityId) return { required: false, missing: false };
    ...
    if (!requires) {
        ...
        if (!requires) return { required: false, missing: false };
    }
...
function warnOnce(currentRecord: any): void {
    ...
    try {
        ...
        alert(
            'This customer requires a credit card on file based on their terms, ' +
            'but no card was found. Please add a Payment Card.'
        );
    }
}

function runCheck(mode: 'warn' | 'block', currentRecord: any): boolean {
    try {
        const res = evaluate(currentRecord);
        if (!res.required) return true;
...
function scheduleWarn(currentRecord: any): void {
...
function scheduleLogPI(currentRecord: any): void {
    if (debounceLogId) window.clearTimeout(debounceLogId);
    debounceLogId = window.setTimeout(() => {
        ...
        try {
            ...
            clog('scheduleLogPI error:', (e as any)?.message || e);
        }
    }, DEBOUNCE_MS);
}
...
const pageInit = (ctx: EntryPoints.Client.pageInitContext): void => {
    try {
...
const postSourcing = (ctx: EntryPoints.Client.postSourcingContext): void => {
    try {
...
const fieldChanged = (_ctx: EntryPoints.Client.fieldChangedContext): boolean => true;

// Final gate — block save if required & missing.
const saveRecord = (ctx: EntryPoints.Client.saveRecordContext): boolean => runCheck('block', ctx.currentRecord);

export = { pageInit, postSourcing, fieldChanged, saveRecord };
