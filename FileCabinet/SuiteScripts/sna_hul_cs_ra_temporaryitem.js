 ...
 * CS script to validate the inventory details of the return authorization
 *
 * Revision History:
 *
...
function(runtime, currentRecord) {
    ...
    function validateLine(scriptContext) {
        var rec = scriptContext.currentRecord;
        var handling = rec.getCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_sna_hul_returns_handling'});
        var itmcatcust = rec.getCurrentSublistValue({sublistId: 'item', fieldId: 'custcol_sna_hul_itemcategory'});
        ...
        if ((tempitemcat == itmcatcust || itmcatcust == allieditemcat || itmcatcust == rackingitemcat || itmcatcust == storageitemcat) && isEmpty(handling)) {
            alert('Enter a Temp Item Returns Handling');
            return false;
        }
    ...
    function saveRecord(scriptContext) {
        ...
        for (var i = 0; i < itemcount; i++) {
            ...
            log.debug({title: 'saveRecord', details: 'line: ' + i + ' | tempcode: ' + tempcode + ' | lineqty: ' + lineqty + ' | itm: ' + itm + ' | handling: ' + handling + ' | itmcatcust: ' + itmcatcust});
