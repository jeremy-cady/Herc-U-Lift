    ...
    (record, runtime, search) => {
        ...
        const getCustomerMatrix = (customerId) => {
            ...
            search.create({
                ...
                columns: [
                    ...
                    search.createColumn({
                        name: "custrecord_salesrep_mapping_override",
                        label: "Override Sales Rep Matrix"
                    }),
                    search.createColumn({
        ...
        const getSalesRep = (salesReps) => {
            ...
            search.create({
                ...
                columns: [search.createColumn({
                    name: "custentity_sna_sales_rep_tran_assignedon",
        ...
        const beforeSubmit = (scriptContext) => {
            ...
            try {
                ...
                let customerMatrix = getCustomerMatrix(customer);
                let addressId = newRecord.getValue({fieldId: "shipaddresslist"});
                let zipCode = customerAddress(customer, addressId);

                for (let line = 0; line < newRecord.getLineCount({sublistId: "item"}); line++) {
                    let itemId = newRecord.getSublistValue({sublistId: "item", fieldId: "item", line});
                    if (!!itemId) {
                        ...
                        log.debug({title: "dataMatrix", details: dataMatrix});
                        ...
                        if (!isEmpty(itemDetails.manufacturer))
                            customerMatrixObject = dataMatrix.find(element => element.manufacturer == itemDetails.manufacturer);

                        log.debug({title: "customerMatrixObject", details: customerMatrixObject});

                        if (!isEmpty(customerMatrixObject)) {
                            let salesReps = customerMatrixObject.salesReps;
                            newRecord.setSublistValue({
                                sublistId: "item", fieldId: "custcol_sna_sales_rep", value: getSalesRep(salesReps), line
                            });
                            newRecord.setSublistValue({
                                sublistId: "item", fieldId: "custcol_sna_sales_rep_matrix",
                                value: customerMatrixObject.id, line
                            });
                            ...
                            if (itemDetails.eligibleForCommission) {
                                newRecord.setSublistValue({
                                    ...
                                    value: itemDetails.eligibleForCommission, line
                                });
                                newRecord.setSublistValue({
                                    sublistId: "item",
                                    fieldId: "custcol_sna_hul_comm_rate",
                                    value: customerMatrixObject.commissionPlanRate, line
                                });
                                newRecord.setSublistValue({
                                    sublistId: "item",
                                    fieldId: "custcol_sna_hul_sales_rep_comm_type",
                                    value: customerMatrixObject.commissionPlanType, line
                                });
                                newRecord.setSublistValue({
                                    sublistId: "item",
                                    fieldId: "custcol_sna_commission_plan",
                                    value: customerMatrixObject.commissionPlan, line
                                });
                            }

                            let amount = Number(newRecord.getSublistValue({
                                sublistId: "item", fieldId: "amount", line
                            }));
                            let costEstimate = Number(newRecord.getSublistValue({
                                sublistId: "item", fieldId: "costestimate", line
                            }));
                            ...
                            if (customerMatrixObject.commissionPlanType == COMMISSION_TYPE.GROSS_MARGIN) {
                                ...
                                newRecord.setSublistValue({
                                    sublistId: "item", fieldId: "custcol_sna_commission_amount",
                                    value: commissionAmount, line
                                });
