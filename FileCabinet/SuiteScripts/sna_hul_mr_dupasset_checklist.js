 ...
 * M/R script to check for duplicate assets in checklist records
 *
 * Revision History:
 *
 ...
 */
    (error, record, runtime, search) => {
        ...
        const readParameters = () => {
            ...
            PARAMS.search = currentScript.getParameter({name: 'custscript_sna_dupe_checklist'});
        }
        ...
        const reduce = (reduceContext) => {
            ...
            let checklisteid = parsedDta.id; // 1 checklist only
            let bodyDupeOfasset = !isEmpty(parsedDta.values['CUSTRECORD_SNA_NXC_RC_ASSET.custrecord_sna_dup_asset']) ? parsedDta.values['CUSTRECORD_SNA_NXC_RC_ASSET.custrecord_sna_dup_asset'][0].value : '';
            let bodyasset = !isEmpty(parsedDta.values.custrecord_sna_nxc_rc_asset) ? parsedDta.values.custrecord_sna_nxc_rc_asset[0].value : '';

            try {
                record.submitFields({type: 'customrecord_sna_hul_rental_checklist', id: checklisteid, values: {'custrecord_sn_hul_mergequipassetchklst': bodyDupeOfasset}});
                log.debug({title: stLoggerTitle, details: checklisteid + ' updated.'}); // 1204
