            ...
            if (salesOrderId) {
                try {
                    ...
                    for (let i = 0; i < lineCount; i++) {
                        // Get the item ID first to check the custom field
                        ...
                        // Check the item's inventory posting group
                        let includeItem = false;
                        try {
                            const itemLookup = search.lookupFields({
                                type: search.Type.ITEM,
                                id: itemId,
                                columns: ['custitem_sna_inv_posting_grp']
                            });
                            
                            const invPostingGrp = itemLookup.custitem_sna_inv_posting_grp;
                            
                            // Check if it's array (multi-select) or single value
                            if (Array.isArray(invPostingGrp)) {
                                // Multi-select field - check if any value is 1 or 2
                                includeItem = invPostingGrp.some(val => 
                                    val.value === '1' || val.value === '2' || 
                                    val.value === 1 || val.value === 2
                                );
                            } else if (invPostingGrp) {
                                // Single value field
                                const grpValue = invPostingGrp.value || invPostingGrp;
                                includeItem = grpValue === '1' || grpValue === '2' || 
                                            grpValue === 1 || grpValue === 2;
                            }
                            
                            // Skip this line if it doesn't meet the criteria
                            if (!includeItem) {
                                continue;
                            }
                            
                        } catch (e) {
                            log.error('Error checking item posting group', {
                                itemId: itemId,
                                error: e.message
                            });
                            // Skip this item if we can't check the field
                            continue;
                        }
