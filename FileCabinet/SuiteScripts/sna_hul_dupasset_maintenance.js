 ...
 * M/R script to check for duplicate assets in maintenace records
 *
 * Revision History:
 *
 ...
 */
    (error, record, runtime, search) => {
        ...
        const readParameters = () => {
            ...
            PARAMS.search = currentScript.getParameter({name: 'custscript_sna_dupe_maintenance'});
        }
        ...
        const reduce = (reduceContext) => {
            ...
            let maintenanceid = parsedDta.id; // 1 maintenance only
            let bodyDupeOfasset = !isEmpty(parsedDta.values['CUSTRECORD_NXC_MR_ASSET.custrecord_sna_dup_asset']) ? parsedDta.values['CUSTRECORD_NXC_MR_ASSET.custrecord_sna_dup_asset'][0].value : '';
            let bodyasset = !isEmpty(parsedDta.values.custrecord_nxc_mr_asset) ? parsedDta.values.custrecord_nxc_mr_asset[0].value : '';

            try {
                log.debug({title: stLoggerTitle, details: 'bodyDupeOfasset: ' + bodyDupeOfasset});
                record.submitFields({type: 'customrecord_nxc_mr', id: maintenanceid, values: {'custrecord_sn_hul_mergequipassetmaint': bodyDupeOfasset}});
                log.debug({title: stLoggerTitle, details: maintenanceid + ' updated.'}); // 442
