 ...
 * M/R script to check for duplicate assets in proj records
 *
 * Revision History:
 *
...
define(['N/error', 'N/record', 'N/runtime', 'N/search'],
    ...
    (error, record, runtime, search) => {
        ...
        const readParameters = () => {
            ...
            PARAMS.search = currentScript.getParameter({name: 'custscript_sna_dupe_proj'});
        }
        ...
        const reduce = (reduceContext) => {
            ...
            let projid = !isEmpty(parsedDta.values['GROUP(internalid)']) ? parsedDta.values['GROUP(internalid)'][0].value : ''; // 1 proj only
            let bodyasset = !isEmpty(parsedDta.values['GROUP(custentity_nx_asset)']) ? parsedDta.values['GROUP(custentity_nx_asset)'][0].value : '';
            let bodyDupeOfasset = !isEmpty(parsedDta.values['GROUP(CUSTENTITY_NX_ASSET.custrecord_sna_dup_asset)']) ? parsedDta.values['GROUP(CUSTENTITY_NX_ASSET.custrecord_sna_dup_asset)'][0].value : '';
            let bodyassets = !isEmpty(parsedDta.values['GROUP(custentity_nxc_project_assets)']) ? parsedDta.values['GROUP(custentity_nxc_project_assets)'] : []; // multi-select
            ...
            try {
                var rec = record.load({type: 'job', id: projid});
                var currbodyAsset = rec.getValue({fieldId: 'custentity_nx_asset'});

                // only set if not null. retain if null
                if (!isEmpty(bodyDupeOfasset) && bodyDupeOfasset != '- None -') {
                    //rec.setValue({fieldId: 'custentity_nx_asset', value: bodyDupeOfasset}); // aduldulao - remove. use custom field instead
                    rec.setValue({fieldId: 'custentity_sn_hul_mergesiteassetproj', value: bodyDupeOfasset});
                ...
                var currbodyAssets = rec.getValue({fieldId: 'custentity_nxc_project_assets'});
                ...
                //rec.setValue({fieldId: 'custentity_nxc_project_assets', value: newbodyAssets}); // aduldulao - remove. use custom field instead
                rec.setValue({fieldId: 'custentity_sn_hul_mergequipassetproj', value: newbodyAssets});
